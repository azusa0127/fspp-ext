...
describe.only( `fsextend Spec`, () => {
  ...
  after(() => {
    ...
    if ( fs.existsSync( LEVEL_2_DIR_TO_BE_CREATED_ABSOLUTE ) )
      fs.rmdirSync( LEVEL_2_DIR_TO_BE_CREATED_ABSOLUTE );
    if ( fs.existsSync( LEVEL_1_DIR_TO_BE_CREATED_ABSOLUTE ) )
      fs.rmdirSync( LEVEL_1_DIR_TO_BE_CREATED_ABSOLUTE );
    if ( fs.existsSync( SANDBOX_DIR_ABSOLUTE ) ) fs.rmdirSync( SANDBOX_DIR_ABSOLUTE );
  } );
  ...
  describe( `ensurePath Spec`, () => {
    ...
    it( `Should do nothing to existing dir.`, async () => {
      ...
      assert.isTrue( fs.existsSync( SANDBOX_DIR_ABSOLUTE ), `Sandbox folder should still be existing.` );
    } );

    it( `Should be able to create Level 1 dummy dir`, async () => {
      assert.isFalse( fs.existsSync( LEVEL_1_DIR_TO_BE_CREATED_ABSOLUTE ), `DummyL1 folder should not exist.` );
      await ensurePath( LEVEL_1_DIR_TO_BE_CREATED_ABSOLUTE );
      assert.isTrue( fs.existsSync( LEVEL_1_DIR_TO_BE_CREATED_ABSOLUTE ), `DummyL1 folder should exist now.` );
    } );
    ...
    it( `Should be able to create Level 2 dummy dir`, async () => {
      ...
      assert.isTrue( fs.existsSync( LEVEL_2_DIR_TO_BE_CREATED_ABSOLUTE ), `DummyL1/DummyL2 folder should exist now.` );
    } );
    ...
    it( `Should fail with non-existing driver`, async () => {
      if ( process.platform === `win32` ) {
        assert.isFalse( await fs.exists( NON_EXISTING_PATH ), `${NON_EXISTING_PATH} Should not exist.` );
        try {
  ...
  describe( `rm Spec`, () => {
    beforeEach(() => {
      ...
      if ( !fs.existsSync( LEVEL_3_TEXT_FILE_ABSOLUTE ) )
        fs.writeFileSync( LEVEL_3_TEXT_FILE_ABSOLUTE, `the quick brown fox jumped over the lazy dog.` );
      if ( !fs.existsSync( LEVEL_3_EMPTY_DIR_ABSOLUTE ) )
        fs.mkdirSync( LEVEL_3_EMPTY_DIR_ABSOLUTE );
    } );

    it( `Should remove single file correctly`, async () => {
      assert.isTrue( fs.existsSync( LEVEL_3_TEXT_FILE_ABSOLUTE ), `The sample text file should exist.` );
      await rm( LEVEL_3_TEXT_FILE_ABSOLUTE );
      assert.isFalse( fs.existsSync( LEVEL_3_TEXT_FILE_ABSOLUTE ), `The sample text file should be removed by now.` );
    } );
    ...
    it( `Should remove empty dir correctly`, async () => {
      ...
      assert.isFalse( fs.existsSync( LEVEL_3_EMPTY_DIR_ABSOLUTE ), `The DummyL3 folder should be removed by now.` );
    } );
    ...
    it( `Should remove Non-Empty dir correctly`, async () => {
      ...
      assert.isFalse( fs.existsSync( LEVEL_1_DIR_TO_BE_CREATED_ABSOLUTE ), `The DummyL1 folder should be removed by now.` );
    } );
  ...
  describe( `cp Spec`, () => {
    ...
    after(() => {
      if ( fs.existsSync( LEVEL_1_DIR_TO_BE_CREATED_ABSOLUTE ) ) rmSync( LEVEL_1_DIR_TO_BE_CREATED_ABSOLUTE );
    ...
    it( `Should be able to copy entire folder`, async () => {
      ...
      assert.equal( fs.readFileSync( path.join( DEST_DUMMYL1_DIR, `textA.txt` ), `utf8` ), fs.readFileSync( SRC_FILE_A_ABSOLUTE, `utf8` ) );
      assert.equal( fs.readFileSync( path.join( DEST_DUMMYL1_DIR, `DummyL2`, `textB.txt` ), `utf8` ), fs.readFileSync( SRC_FILE_B_ABSOLUTE, `utf8` ) );
    } );
